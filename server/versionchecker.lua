        ...
        PerformHttpRequest(repositoryUrl, function(errorCode, newestVersion, headers)
            if errorCode == 200 then
                ...
                if newestVersion ~= currentVersion then
                    print(string.format(
                        '\n\n^1[WARNING]^3 The script is outdated v(%s). Please download the latest version v(%s) from my GitHub repository: ^9%s/releases\n\n',
                        currentVersion, newestVersion, 'https://github.com/masked1337/masked-smallresources'))
                end
            else
